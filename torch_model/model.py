...
class BiLSTM(torch.nn.Module):
    ...
    def forward(self, x, x_length):
        ...
        out = self.linear_head(rnn_out)  
        return out

class BiLSTM_CRF(torch.nn.Module):
    def __init__(self, embedding_layer, rnn_layer, linear_head, crf_layer):
    ...
    def forward(self, x, x_length, labels, mask):
        embed = self.embedding(x) 
        rnn_out = self.rnn(embed, x_length) 
        out = self.linear_head(rnn_out)  
        loss = -self.crf(out, labels, mask)
        return out, loss